...
- A package, credit, or ledger record is not proof of a live payment. No payment provider is live in production.
- PayTR is built on the frontend and switched off. The payment route, hosted-iframe step, return pages and entry points are on main behind `NEXT_PUBLIC_PAYTR_ENABLED`, which is off by default; backend `PAYTR_ENABLED` is what authorises a real charge, the backend PayTR pull request is still open, and no real test-mode transaction has been completed. Turning production on is a separate decision with its own preconditions: `docs/payments/PAYTR_RELEASE_RUNBOOK.md`.
