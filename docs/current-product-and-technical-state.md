...
- PayTR is under development, not activated. The frontend currently holds only its API layer, types and an off-by-default `NEXT_PUBLIC_PAYTR_ENABLED` build flag; backend `PAYTR_ENABLED` is what authorises a real charge, and switching production on is a separate operational decision. Plan and contract: `docs/payments/`.
