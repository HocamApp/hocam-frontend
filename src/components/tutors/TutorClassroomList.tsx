  ...
  return <WorkspacePageShell title="Sınıfım" width="wide">
    ...
    {data.isLoading ? <ClassroomLoading /> : data.isError ? <ClassroomError message="Öğrencilerin yüklenemedi. Yeniden deneyebilirsin." onRetry={data.retry} /> : !data.profile.data?.is_verified ? <p>Öğrenci alanına erişmek için hoca doğrulamanı tamamlamalısın.</p> : <>
      <div className="space-y-4">
        <div className="relative max-w-md"><label htmlFor="classroom-search" className="sr-only">Öğrenci ara</label><MagnifyingGlass className="pointer-events-none absolute left-3 top-3 h-5 w-5 text-ink-mid" aria-hidden="true" /><Input id="classroom-search" type="search" value={search} onChange={e => setSearch(e.target.value)} placeholder="Ad veya e-posta ile ara" className="h-11 rounded-input border-line bg-surface pl-10 text-base" /></div>
      ...
      {rows.length ? <ul className="divide-y divide-line overflow-hidden rounded-card border border-line bg-surface">{rows.map(entry => <li key={entry.student.id}><Link href={`/dashboard/tutor/classroom/${entry.student.id}`} className="grid min-w-0 gap-4 p-4 transition-colors duration-[120ms] hover:bg-paper focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-ink sm:p-6 lg:grid-cols-[minmax(0,1.2fr)_minmax(0,1fr)_minmax(0,.7fr)_auto] lg:items-center">
        <div className="flex min-w-0 items-center gap-3"><StudentAvatar student={entry.student} /><div className="min-w-0"><h2 className="break-words font-medium">{studentName(entry.student)}</h2>{entry.student.display_name && <p className="break-all text-small text-ink-mid">{entry.student.email}</p>}{entry.currentLesson && <p className="mt-1 text-small font-medium">Ders devam ediyor</p>}</div></div>
