...
/**
 * Timeline primitives (Nyxb UI, via 21st.dev). Added verbatim apart from
 * formatting so it matches the rest of `components/ui`.
 *
 * The layout is a CSS grid per item: the dot sits in column 2 row 1, the
 * connecting line in column 2 row 2, and heading/content in column 3. That is
 * why the sub-components must stay siblings inside a `TimelineItem` — wrapping
 * them in a div breaks the grid placement.
 *
 * Note `TimelineHeading` truncates to one line by default. Pass
 * `className="line-clamp-none whitespace-normal"` for headings that need to
 * wrap.
 */
