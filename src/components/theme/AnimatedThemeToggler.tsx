...
export const AnimatedThemeToggler = ({
  ...
  // Always start at the SSR-safe default (light/Moon), matching what the
  // server renders (window is unavailable there). THEME_INIT_SCRIPT already
  // applies the real `dark` class to <html> before first paint, avoiding a
  // page-wide flash — but reading it here during the lazy-initializer would
  // make the client's hydration-time render differ from the server's HTML
  // for anyone with dark mode persisted, causing a hydration mismatch. We
  // sync to the real value in the effect below instead, which runs after
  // hydration completes and is a normal client-only update, not part of the
  // hydration diff.
