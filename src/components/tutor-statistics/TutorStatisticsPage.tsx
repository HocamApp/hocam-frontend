...
const criteriaLabels: Record<string, string> = Object.fromEntries(
  reviewCriteria.map(({ key, label }) => [key, label]),
);
...
function Overview({ data }: { data: TutorStatisticsResponse }) {
  ...
  return <div className="space-y-6">
    ...
    <ChartCard title="Tamamlanan dersler" description="Tamamlanan kayıtların planlanan süresi gösterilir; canlı bağlantı süresi değildir." action={<div role="group" aria-label="Ders grafiği ölçüsü" className="flex gap-1">{(["count", "minutes"] as const).map(mode => <Button key={mode} size="sm" variant="outline" aria-pressed={activityMode === mode} onClick={() => setActivityMode(mode)} className={activityMode === mode ? "border-ink bg-ink text-paper hover:bg-ink hover:text-paper" : "border-line"}>{mode === "count" ? "Ders" : "Dakika"}</Button>)}</div>}>
      <LineChart title="Tamamlanan dersler" points={data.lesson_activity.map(row => ({ label: row.period_start, value: row[activityKey] }))} valueLabel={value => activityMode === "count" ? `${value} ders` : `${value} dakika`} />
    </ChartCard>

    <div className="grid gap-6 lg:grid-cols-2">
      <ChartCard title="Ders durumları" description="Seçili dönemde başlayan tüm ders kayıtları."><HorizontalBars title="Ders durumları" rows={data.lesson_statuses.map(row => ({ label: statusLabel(row.status), value: row.count }))} /></ChartCard>
      <ChartCard title="Ders alanları" description="Tamamlanan derslerin konu dağılımı."><HorizontalBars title="Ders alanları" rows={data.subjects.map(row => ({ label: `${row.name} · ${row.exam_type}`, value: row.completed_count }))} valueLabel={value => `${value} ders`} /></ChartCard>
    </div>

    <div className="grid gap-6 lg:grid-cols-2">
      <ChartCard title="Öğrenci gelişimi" description={`${data.students.relationships} öğrenciyle ders ilişkisi · ${data.students.new} yeni öğrenci`}><LineChart title="Yeni öğrenciler" points={data.students.activity.map(row => ({ label: row.period_start, value: row.new_students }))} valueLabel={value => `${value} öğrenci`} /></ChartCard>
      <ChartCard title="Değerlendirme eğilimi" description={reviewAverage === null ? "Değerlendirme gelmediğinde sıfır çizgisi üretilmez." : `${data.summary.reviews.count} değerlendirme üzerinden.`}><LineChart title="Değerlendirme eğilimi" points={data.reviews.trend.map(row => ({ label: row.period_start, value: row.average }))} valueLabel={value => `${value.toFixed(2)} / 5`} />
    ...
    <ChartCard title="Güvenilirlik" description="İptal nedenleri ve ders dışı mesajlara ilk 24 saatte verilen yanıtlar.">
      <div className="grid gap-5 sm:grid-cols-3"><div><p className="text-caption text-ink-mid">Hoca kaynaklı kaçırılan ders</p><strong className="mt-1 block text-h2-m">{data.reliability.missed_lessons}</strong></div><div><p className="text-caption text-ink-mid">24 saatte yanıt</p><strong className="mt-1 block text-h2-m">{percent(data.reliability.reply_rate_24h.rate)}</strong><p className="text-caption text-ink-mid">{data.reliability.reply_rate_24h.answered}/{data.reliability.reply_rate_24h.total} ölçülen konuşma dönüşü</p></div><div><p className="text-caption text-ink-mid">Toplam iptal</p><strong className="mt-1 block text-h2-m">{data.reliability.cancellations.reduce((total, row) => total + row.count, 0)}</strong></div></div>
      <div className="mt-5 border-t border-line pt-5"><HorizontalBars title="İptal nedenleri" rows={data.reliability.cancellations.map(row => ({ label: CANCELLATION_LABELS[row.reason] ?? row.reason, value: row.count }))} /></div>
    </ChartCard>

    {data.coaching && <section className="space-y-5 border-t-2 border-ink pt-7"><div><p className="text-caption font-semibold uppercase tracking-[.12em] text-pink">Ayrı hizmet alanı</p><h2 className="mt-2 text-h2-m font-bold sm:text-h2">Koçluk görünümü</h2><p className="mt-1 text-small text-ink-mid">Koçluk görüşmeleri ders istatistiklerine eklenmeden ayrı hesaplanır.</p></div><div className="grid gap-3 sm:grid-cols-3"><MetricCard label="Toplam görüşme" value={String(data.coaching.summary.total_sessions)} detail="Seçili dönemde planlanan" icon={<CalendarDots size={22} />} /><MetricCard label="Tamamlanan görüşme" value={String(data.coaching.summary.completed_sessions)} detail="Yayınlanmış raporla tamamlanan" icon={<ChalkboardTeacher size={22} />} /><MetricCard label="Koçluk öğrencisi" value={String(data.coaching.summary.students)} detail="Bu dönemde görüşmesi olan" icon={<Users size={22} />} /></div><div className="grid gap-6 lg:grid-cols-2"><ChartCard title="Koçluk görüşmeleri"><LineChart title="Tamamlanan koçluk görüşmeleri" points={data.coaching.activity.map(row => ({ label: row.period_start, value: row.completed_count }))} valueLabel={value => `${value} görüşme`} /></ChartCard><ChartCard title="Koçluk durumları"><HorizontalBars title="Koçluk durumları" rows={data.coaching.statuses.map(row => ({ label: statusLabel(row.status), value: row.count }))} /></ChartCard></div></section>}
  </div>;
}

const recordKinds: Array<{ value: IncomeRecordKind; label: string }> = [{ value: "packages", label: "Paket kayıtları" }, { value: "earnings", label: "Koçluk hakedişleri" }, { value: "payouts", label: "Ödeme partileri" }];
function Income({ data, selection }: { data: TutorIncomeResponse; selection: StatisticsSelection }) {
  ...
  return <div className="space-y-6">
    <section className="grid gap-3 lg:grid-cols-3">
      <MetricCard label="Ders paketi tutarları" value={formatMinor(data.packages.recorded_amount_minor)} detail={`${data.packages.count} paket kaydındaki tutar; tahsilat veya hoca ödemesi kanıtı değildir.`} icon={<CurrencyCircleDollar size={22} />} />
      <MetricCard label="Ders kazancı" value={data.lesson_earnings.available && data.lesson_earnings.amount_minor !== null ? formatMinor(data.lesson_earnings.amount_minor) : "Henüz hesaplanamıyor"} detail="Ders ödeme kaynağı gerçek hoca ödemesini ayırmadığı için tutar gösterilmiyor." icon={<BookOpen size={22} />} />
      <MetricCard label="Koçluk hakedişi" value={data.coaching ? formatMinor(data.coaching.entitlement_amount_minor) : "Kapalı"} detail={data.coaching ? "Teslim edilen koçluk hizmetlerinden doğan hak; ödeme toplamı değildir." : "Koçluk özelliği kapalı."} icon={<ChalkboardTeacher size={22} />} />
    </section>
    <ChartCard title="Ders paketi tutarları" description="Paket kayıtları oluşturulma tarihine ve güncel durumuna göre gösterilir."><LineChart title="Ders paketi tutarları" points={data.packages.activity.map(row => ({ label: row.period_start, value: row.recorded_amount_minor }))} valueLabel={value => formatMinor(value)} /><div className="mt-5 border-t border-line pt-5"><HorizontalBars title="Paket durumları" rows={data.packages.statuses.map(row => ({ label: `${statusLabel(row.status)} · ${formatMinor(row.recorded_amount_minor)}`, value: row.count }))} valueLabel={value => `${value} kayıt`} /></div></ChartCard>
    {data.coaching && <section className="space-y-5 border-t-2 border-ink pt-7"><div><p className="text-caption font-semibold uppercase tracking-[.12em] text-pink">Koçluk finans akışı</p><h2 className="mt-2 text-h2-m font-bold sm:text-h2">Hakediş ve ödeme durumu</h2><p className="mt-1 max-w-3xl text-small text-ink-mid">Hakediş, ödeme partisi ve ödenen tutar birbirinden ayrı gösterilir. Aynı hakediş bu başlıklar toplanarak yeniden sayılmaz.</p></div><div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3"><MetricCard label="Fon bekleyen" value={formatMinor(data.coaching.status_amounts_minor.eligible_unfunded ?? 0)} detail="Tahsilatla fonlanmamış hak" icon={<Clock size={22} />} /><MetricCard label="Hakediş bekliyor" value={formatMinor(data.coaching.status_amounts_minor.pending ?? 0)} detail="Partiye alınabilir mevcut durum" icon={<CalendarDots size={22} />} /><MetricCard label="İncelemede" value={formatMinor(data.coaching.status_amounts_minor.on_hold ?? 0)} detail="Uyuşmazlık nedeniyle bekletilen hak" icon={<ChatCircle size={22} />} /><MetricCard label="Ters kayıt" value={formatMinor(data.coaching.status_amounts_minor.reversed ?? 0)} detail="İptal edilen hakediş kaydı" icon={<CurrencyCircleDollar size={22} />} /><MetricCard label="Ödemeye hazır" value={formatMinor(data.coaching.batched_ready_amount_minor)} detail="READY partileri; henüz ödenmiş değildir" icon={<CurrencyCircleDollar size={22} />} /><MetricCard label="Ödenen" value={formatMinor(data.coaching.paid_amount_minor)} detail="PAID olarak işaretlenmiş partiler" icon={<CurrencyCircleDollar size={22} />} /></div><ChartCard title="Koçluk hakediş eğilimi"><LineChart title="Koçluk hakedişleri" points={data.coaching.activity.map(row => ({ label: row.period_start, value: row.entitlement_amount_minor }))} valueLabel={value => formatMinor(value)} /></ChartCard>{data.coaching.payout_batches.length > 0 && <ChartCard title="Aylık ödeme partileri"><ul className="divide-y divide-line">{data.coaching.payout_batches.map(batch => <li key={batch.id} className="flex flex-wrap items-center justify-between gap-3 py-3 first:pt-0 last:pb-0"><span>{formatDate(batch.local_month)} · {statusLabel(batch.status)}</span><strong>{formatMinor(batch.total_amount_minor)}</strong></li>)}</ul></ChartCard>}</section>}
...
function Reviews({ selection }: { selection: StatisticsSelection }) {
  ...
  return <section aria-labelledby="tutor-review-records-title" className="space-y-4">
    <div><h2 id="tutor-review-records-title" className="text-h2-m font-bold sm:text-h2">Öğrenci değerlendirmeleri</h2><p className="mt-1 text-small text-ink-mid tabular-nums">{records.data.count} değerlendirme · yalnızca sana ait geri bildirimler</p></div>
...
export function TutorStatisticsPage() {
  ...
  return <WorkspacePageShell title="İstatistiklerim" width="wide">
    <div className="flex flex-col gap-5 border-b border-line pb-6 lg:flex-row lg:items-end lg:justify-between"><div className="min-w-0"><p className="max-w-2xl text-small text-ink-mid">Ders ve koçluk etkinliğini zaman içinde incele. Tüm tarihler İstanbul saatine göre hesaplanır.</p><div role="tablist" aria-label="İstatistik bölümü" className="mt-4 flex max-w-full gap-2 overflow-x-auto pb-1">{([{value:"overview",label:"Genel bakış"},{value:"reviews",label:"Değerlendirmeler"},{value:"income",label:"Gelir"}] as const).map(item => <Button key={item.value} role="tab" aria-selected={selection.tab === item.value} variant="outline" onClick={() => navigate({ ...selection, tab: item.value })} className={selection.tab === item.value ? "border-ink bg-ink text-paper hover:bg-ink hover:text-paper" : "border-line"}>{item.label}</Button>)}</div></div><div role="group" aria-label="İstatistik dönemi" className="flex flex-wrap gap-2">{periods.map(period => <Button key={period.value} variant="outline" size="sm" aria-pressed={selection.period === period.value} onClick={() => changePeriod(period.value)} className={selection.period === period.value ? "border-pink bg-pink text-white hover:bg-pink-deep hover:text-white" : "border-line"}>{period.label}</Button>)}</div></div>
    ...
    <p className="text-caption text-ink-mid">{formatDate(selection.from)} – {formatDate(selection.to)} · İstanbul</p>
