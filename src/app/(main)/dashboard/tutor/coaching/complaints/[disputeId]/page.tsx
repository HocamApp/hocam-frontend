...
function Detail() {
  ...
  if (!query.data) return <CoachingEmptyState title="Sorun bildirimi bulunamadı" description="Bu kayıt artık erişilebilir değil veya sana ait değil." />;
  const dispute = query.data;
  return <div className="space-y-5"><Card><CardContent className="space-y-2 py-5"><div className="flex justify-between gap-3"><div><CoachingSectionHeading>Sorun özeti</CoachingSectionHeading><p className="text-sm text-muted-foreground">{coachingDisputeCategoryLabel(dispute.category)}</p></div><Badge>{coachingDisputeStatusLabel(dispute.status)}</Badge></div><p className="text-sm text-muted-foreground">Öğrenci açıklaması, personel notları ve sunulmayan kanıtlar gizlidir.</p></CardContent></Card>
...
export default function TutorCoachingComplaintDetailPage() { return <CoachingRecordGuard><CoachingPageShell title="Koçluk sorun bildirimi" width="wide" currentHref="/dashboard/tutor/coaching/complaints" audience="tutor"><Detail /></CoachingPageShell></CoachingRecordGuard>; }
