...
function Detail() {
  ...
  return <div className="space-y-5"><Card><CardContent className="space-y-2 py-5"><div className="flex justify-between gap-3"><div><h1 className="text-xl font-semibold">Koçluk başvurusu</h1><p className="text-sm text-muted-foreground">{dispute.category}</p></div><Badge>{dispute.status}</Badge></div><p className="text-sm text-muted-foreground">Öğrenci açıklaması, personel notları ve sunulmayan kanıtlar gizlidir.</p></CardContent></Card>
...
export default function TutorCoachingComplaintDetailPage() { return <CoachingRecordGuard><main className="mx-auto max-w-3xl px-4 py-8"><Detail /></main></CoachingRecordGuard>; }
