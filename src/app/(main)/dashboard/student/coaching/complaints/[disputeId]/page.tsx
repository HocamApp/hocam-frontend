...
function Detail() {
  ...
  if (!query.data) return <CoachingEmptyState title="Sorun bildirimi bulunamadı" description="Bu kayıt artık erişilebilir değil veya hesabına ait değil." />;
  const dispute = query.data;
  return <div className="space-y-5"><Card><CardContent className="space-y-3 py-5"><div className="flex justify-between gap-2"><div><CoachingSectionHeading>Sorun özeti</CoachingSectionHeading><p className="text-sm text-muted-foreground">{coachingDisputeCategoryLabel(dispute.category)}</p></div><Badge>{coachingDisputeStatusLabel(dispute.status)}</Badge></div><p className="whitespace-pre-wrap text-sm">{dispute.description}</p></CardContent></Card>
    ...
    <Card><CardContent className="space-y-3 py-5"><CoachingSectionHeading level="subsection">Kanıtlar</CoachingSectionHeading><input type="file" onChange={(event) => { const file = event.target.files?.[0]; if (file) upload.mutate(file); }} disabled={upload.isPending} />
      ...
      <p className="text-xs text-muted-foreground">Tarama tamamlanmamış, kullanılamayan veya reddedilen kanıtlar karara esas alınmaz.</p></CardContent></Card>
...
export default function StudentCoachingComplaintDetailPage() { return <RouteGuard requireAuth requireRole="student"><CoachingPageShell title="Koçluk sorun bildirimi" width="narrow" currentHref="/dashboard/student/coaching/complaints" audience="student"><Detail /></CoachingPageShell></RouteGuard>; }
